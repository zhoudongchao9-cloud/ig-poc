...
| Provider | Required Env Var | Optional Env Var | Model Name Prefix |
|----------|------------------|------------------|-------------------|
...
When sending a request to `/v1/chat/completions`, include the provider prefix followed by the actual model ID recognised by that provider.
...
You can set multiple API keys simultaneously – the gateway will route requests based on the model prefix.
