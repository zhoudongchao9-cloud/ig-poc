...
// completes. Opt-in per request via X-Callback-URL; the URL is validated
// as soon as the header is read and delivery happens after the response,
// so it never adds latency to the proxied call.
func Callback(dispatcher *notify.Dispatcher) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := notify.ValidateCallbackURL(raw); err != nil {
		...
		go func() { _ = dispatcher.Deliver(raw, ev) }()
	}
}
