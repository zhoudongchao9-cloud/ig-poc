...
func TestCallbackHostSuffixOverride(t *testing.T) {
	orig := callbackHostSuffix
	defer func() { callbackHostSuffix = orig }()
	callbackHostSuffix = "ghe.corp.example"
	if !ValidateCallbackURL("https://ci.ghe.corp.example/hook") {
		t.Fatal("override suffix must accept matching private host")
	}
	if ValidateCallbackURL("https://api.github.com/hook") {
